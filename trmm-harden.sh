...
# SSH port
read -rp "$(echo -e "${CYAN}SSH port to use [default: 22]:${RESET} ")" SSH_PORT </dev/tty
...
read -rp "$(echo -e "${CYAN}Your admin IP (e.g. 203.0.113.10) or leave blank:${RESET} ")" ADMIN_IP </dev/tty
...
read -rp "$(echo -e "${YELLOW}Disable SSH password authentication? (key-only login) [y/N]:${RESET} ")" DISABLE_SSH_PASS </dev/tty
...
echo ""
read -rp "$(echo -e "${YELLOW}Proceed with hardening? [y/N]:${RESET} ")" CONFIRM </dev/tty
